      ...
      <el-table
        ...
        <el-table-column label="状态">
          <template v-slot:default="scope">
            <span v-if="scope.row.state === 1" style="color: red">异常</span>
...
export default {
  ...
  methods: {
    // 获取用户列表
    async getUserList() {
      const {data: res} = await this.$http.post('/user/getdata', this.queryInfo)
    ...
    addUser() {
      this.$refs.userInfoRef.validate(async valid => {
        if (!valid) return
        const {data: res} = await this.$http.post('/user/register', this.userInfo)
    ...
    updateUser(id, username, nickname, mobile, state) {
      ...
      this.updateUserInfo.state  = state
      this.editDialogVisible = true
    },
    async editUser() {
      await this.$http.post('/user/editUserInfo', this.updateUserInfo)
